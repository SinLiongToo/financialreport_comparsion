...
const I18N_DICT = {
    en: {
        ...
        header_updated: "Updated: 2026-09-16",
        ...
        insights_filter_ai_defense: "🤖 Frontier AI & Defense",
        insights_filter_foundry: "⚡ Foundry & IDM",
        insights_filter_fabless: "🧠 Fabless & IC Design",
        insights_filter_equipment: "🔬 Fab Equipment & Materials",
        insights_filter_testing: "🧪 Testing, ATE & OSAT",
        insights_filter_hardware: "💻 System OEMs & AI Hardware",
        insights_filter_hyperscale: "☁️ Hyperscalers & Enterprise Cloud",
        insights_search_placeholder: "Search notes by keyword (e.g. TSMC, NVIDIA, Wiwynn, operating margin, R&D, Palantir)...",
        ...
        filter_country_fr: "🇫🇷 France (AI.PA)",
    ...
    zh: {
        ...
        header_updated: "更新日期：2026-09-16",
        ...
        insights_header_title: "產業戰略洞察與深度研究筆記庫",
        insights_header_subtitle: "結構化量化指標與質化分析：涵蓋晶圓製造、IC設計、設備材料、封裝測試、AI伺服器硬體、雲端巨頭與前沿軍工",
        insights_filter_all: "全部研究筆記",
        insights_filter_ai_defense: "🤖 前沿 AI 與國防軍工",
        insights_filter_foundry: "⚡ 晶圓製造 / IDM",
        insights_filter_fabless: "🧠 IC設計 / Fabless",
        insights_filter_equipment: "🔬 半導體設備與關鍵材料",
        insights_filter_testing: "🧪 半導體測試、探針卡與封測",
        insights_filter_hardware: "💻 伺服器代工與硬體系統",
        insights_filter_hyperscale: "☁️ 雲端巨頭與企業軟體",
        insights_search_placeholder: "搜尋筆記關鍵字 (例如：台積電, NVIDIA, 廣達, 緯穎, 測試, 資本支出, Palantir)...",
        btn_copy_note_md: "複製 Markdown 筆記",
...
function setupEventListeners() {
    ...
    // ---------------------------------------------------------------------------
    // 1-Click Collapse / Expand Toggle for Multi-Company Selector Grid
    // ---------------------------------------------------------------------------
    const toggleCompareBtn = document.getElementById("toggleCompareGridBtn");
    const compareGridBody = document.getElementById("compareGridCollapsibleBody");
    const toggleGridIcon = document.getElementById("toggleGridIcon");
    const toggleGridLabel = document.getElementById("toggleGridLabel");

    if (toggleCompareBtn && compareGridBody) {
        toggleCompareBtn.addEventListener("click", (e) => {
            e.preventDefault();
            e.stopPropagation();
            const isHidden = compareGridBody.classList.contains("hidden") || compareGridBody.style.display === "none";
            const dict = I18N_DICT[CURRENT_LANGUAGE] || I18N_DICT.en;

            if (!isHidden) {
                // Collapse
                compareGridBody.classList.add("hidden");
                compareGridBody.style.display = "none";
                if (toggleGridIcon) toggleGridIcon.className = "fa-solid fa-chevron-down";
                if (toggleGridLabel) {
                    toggleGridLabel.textContent = dict.btn_expand_grid || "Expand Grid";
                    toggleGridLabel.setAttribute("data-i18n", "btn_expand_grid");
                }
                toggleCompareBtn.className = "bg-amber-600/30 hover:bg-amber-600/50 text-xs text-amber-300 hover:text-amber-200 px-3 py-1.5 rounded-lg transition-all font-semibold flex items-center gap-1.5 border border-amber-500/50 shadow-sm cursor-pointer";
            } else {
                // Expand
                compareGridBody.classList.remove("hidden");
                compareGridBody.style.display = "block";
                if (toggleGridIcon) toggleGridIcon.className = "fa-solid fa-chevron-up";
                if (toggleGridLabel) {
                    toggleGridLabel.textContent = dict.btn_collapse_grid || "Collapse Grid";
                    toggleGridLabel.setAttribute("data-i18n", "btn_collapse_grid");
                }
                toggleCompareBtn.className = "bg-slate-700 hover:bg-slate-600 text-xs text-amber-300 hover:text-amber-200 px-3 py-1.5 rounded-lg transition-all font-semibold flex items-center gap-1.5 border border-slate-600 shadow-sm cursor-pointer";
            }
        });
    }
...
async function loadComparisonData() {
    const rawChecked = Array.from(document.querySelectorAll(".compare-chk:checked")).map(cb => cb.value.toLowerCase());
    ...
    const checkedBoxes = [];
    rawChecked.forEach(t => {
        const canon = FinancialMetricsExtractor_canonical_ticker(t).toLowerCase();
        if (!seenCanons.has(canon)) {
            seenCanons.add(canon);
            checkedBoxes.push(canon);
        }
    });
...
function renderComparisonTableRows(companiesData, tickersList) {
    ...
    // Deduplicate tickers by canonical key
    const seenTickers = new Set();
    const uniqueSortedTickers = [];
    sortedTickers.forEach(t => {
        const canon = FinancialMetricsExtractor_canonical_ticker(t).toLowerCase();
        if (!seenTickers.has(canon)) {
            seenTickers.add(canon);
            uniqueSortedTickers.push(t);
        }
    });
