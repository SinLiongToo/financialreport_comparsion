...
- **Autonomous Daily GitHub Actions Cron (`.github/workflows/daily_stock_update.yml`)**:
  ...
  - Automatically executes `validate_company.py all` and `export_standalone.py` to compile `docs/index.html` and `standalone_dashboard.html`, committing and pushing directly to ensure global zero-maintenance GitHub Pages & Cloudflare Pages synchronization.
